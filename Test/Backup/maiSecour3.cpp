...
VSCOM vscom;
DonneesCANClass can("MatTest","CAN.json","172.18.110.111",3000);
ServeurTCP serveur("0.0.0.0",2085);
//JSONFile json("TestUser","CAN.json");
ClientTCP client;
int nbTrames[2048];
can.tabDonneesFormatee;
can.tabDonnees;     //11 bits
...
int main(){
	...
	{	can.ChargerDonneesCAN("DonneeCAN.log");
	}
	while(OK)
	{   	
			can.affichageDonneeCAN();
			if(testUnitaire=="CAN" || testUnitaire=="COMPLET")
			{	can.SauvegarderDonneesCAN("DonneeCAN.log");
			}
			usleep(2000000);
			//EnvoyerDonneesAuServeurREST("172.18.110.111:3000");		
			can.EnvoyerDonneesAuServeurREST();	
